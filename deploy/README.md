...
After the API is available, import `deploy/render-frontend.yaml` as a third Blueprint. It builds the `frontend` directory as a free static site and prompts for the actual API HTTPS URL in `VITE_API_BASE_URL`. Update the API `CORS_ORIGIN` to the exact assigned frontend origin and verify browser flows. Static hosting does not need a paid web-service instance. Build filters keep unrelated services from rebuilding when another service changes.
