...
The root `render.yaml` defines two free Docker web services, one free cache, a shared generated service key, and generated JWT/admin secrets. It prompts for database/Kafka credentials, the catalogue HTTPS URL and the final Vercel origin. Catalogue and API are separate services. Existing Render resource names must be checked before importing the blueprint to avoid modifying an unrelated service.
