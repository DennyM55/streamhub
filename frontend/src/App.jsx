...
import './App.css'
import { demoApi, DEMO_MODE } from './demo.js'

const API = (import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:8080').replace(/\/$/, '')

async function api(path, token, options = {}) {
  if (DEMO_MODE) return demoApi(path, token, options)
...
function MoviePlayer({ movie, resumeSeconds, historyLoading, onProgress }) {
  ...
  useEffect(() => {
    ...
    const position = resumeSeconds >= duration - 2 ? 0 : Math.max(0, Math.min(resumeSeconds || 0, duration))
...
export default function App() {
  ...
  async function saveProgress(e) {
    ...
    try {
      ...
      setSavedMovies(old => ({ ...old, [selected.id]: selected })); setMessage(DEMO_MODE ? 'Watch progress saved in this browser.' : 'Watch progress saved to your account.')
    } catch (err) {
      if (sessionEpoch.current === requestSession) {
        if (err.status === 401) { signOut(); setMessage('Your session has expired. Please sign in again.') }
